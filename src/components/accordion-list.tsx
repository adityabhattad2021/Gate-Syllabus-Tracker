...
export default function AccordionList({
    ...
    const [isChecked, setIsChecked] = useState<boolean[]>(subjectData.chapters.map(() => false));
        ...
        <Accordion type="multiple" className="w-full space-y-4">
            {subjectData.chapters.map((chapter, index) => (
                ...
                >
                            ...
                            <AccordionTrigger disabled={chapter.topics.length === 0} className={cn("z-50", chapter.topics.length === 0 && "cursor-not-allowed")}>
                                <svg
                                    ...
                                    className={`h-5 w-5 transition-transform duration-200 ${
                                        chapter.topics.length === 0 ? 'text-zinc-600' : 'text-zinc-400'
                                    }`}
                                >
                                    <path d="M6 9l6 6 6-6" />
                                </svg>
                    ...
                    {chapter.topics.length > 0 && (
                        <AccordionContent className="px-6 py-4 bg-gary-800 border-t border-zinc-700">
                            <ul className="space-y-3">
                                {chapter.topics.map((topic, topicIndex) => (
                                    <li key={topicIndex} className="text-base text-zinc-200 hover:text-secondary transition-colors duration-200 bg-zinc-800 rounded-md p-3 flex items-center justify-between">
                                        <span>{topic}</span>
                                        <div className="flex items-center gap-2">
                                            <Checkbox className="z-50 focus:ring-secondary focus:ring-offset-2 focus:ring-offset-background  border-2 border-secondary" />
                                            <Link
                                                target="_blank"
